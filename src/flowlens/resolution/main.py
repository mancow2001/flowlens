...
Runs resolution workers to build dependency graph from flows.
"""
...
async def main() -> None:
    """Main entry point for resolution service."""
    ...
    logger.info(
        "Starting Resolution Service",
        version=settings.app_version,
        environment=settings.environment,
        worker_count=settings.resolution.worker_count,
    )
    ...
    # Create workers
    workers = [
        ResolutionWorker(settings.resolution)
        for _ in range(settings.resolution.worker_count)
    ]
