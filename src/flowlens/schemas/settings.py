...
SETTINGS_SECTIONS: list[SettingsSectionInfo] = [
    ...
    SettingsSectionInfo(
        ...
        fields=[
            ...
            FieldMetadata(
                name="stale_dependency_cleanup_days",
                label="Stale Dependency Cleanup",
                description="Days of inactivity before automatically closing stale dependencies",
                field_type=FieldType.INTEGER,
                env_var="RESOLUTION_STALE_DEPENDENCY_CLEANUP_DAYS",
                default=30,
                min_value=1,
                max_value=365,
            ),
            FieldMetadata(
                name="stale_asset_cleanup_days",
                label="Stale Asset Cleanup",
                description="Days of inactivity before automatically soft-deleting stale assets",
                field_type=FieldType.INTEGER,
                env_var="RESOLUTION_STALE_ASSET_CLEANUP_DAYS",
                default=30,
                min_value=1,
                max_value=365,
            ),
    ...
    SettingsSectionInfo(
        ...
        fields=[
            ...
            FieldMetadata(
                name="api_key",
                label="API Key",
                description="Your API key for the selected provider (optional for local providers)",
                field_type=FieldType.SECRET,
                env_var="LLM_API_KEY",
                required=False,
                is_secret=True,
            ...
            FieldMetadata(
                name="base_url",
                label="Base URL",
                description="Custom API base URL for OpenAI-compatible providers (e.g., http://localhost:11434/v1 for Ollama, http://localhost:1234/v1 for LM Studio)",
                field_type=FieldType.STRING,
            ...
            FieldMetadata(
                ...
                description="Override the default model (leave empty to use provider default: claude-sonnet-4-20250514 for Anthropic, gpt-4o for OpenAI, llama3.2 for local)",
                field_type=FieldType.STRING,
                env_var="LLM_MODEL",
                required=False,
            ),
