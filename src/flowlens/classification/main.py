...
Runs classification workers to auto-classify assets based on behavioral features.
"""
...
async def main() -> None:
    ...
    logger.info(
        ...
        version=settings.app_version,
        environment=settings.environment,
        worker_count=settings.classification.worker_count,
        poll_interval_ms=settings.classification.poll_interval_ms,
        confidence_threshold=settings.classification.auto_update_confidence_threshold,
    )
    ...
    # Create workers
    workers = [
        ClassificationWorker(settings.classification)
        for _ in range(settings.classification.worker_count)
    ]
